...
getTab();function
getPanel();function
getId();}}namespace Nette\Caching{use
Nette;interface
...
function
save($config,$file,$section=NULL);}}namespace Nette\Database{use
Nette;interface
...
render(Form$form);}}namespace Nette\Mail{use
Nette;interface
IMailer{function
...
translate($message,$count=NULL);}}namespace Nette\Web{use
Nette;interface
IHttpRequest{const
...
run(){$httpRequest=$this->getHttpRequest();$httpResponse=$this->getHttpResponse();$session=$this->getSession();if(!$session->isStarted()&&$session->exists()){$session->start();}if($this->allowedMethods){$method=$httpRequest->getMethod();if(!in_array($method,$this->allowedMethods,TRUE)){$httpResponse->setCode(Nette\Web\IHttpResponse::S501_NOT_IMPLEMENTED);$httpResponse->setHeader('Allow',implode(',',$this->allowedMethods));echo'<h1>Method '.htmlSpecialChars($method).' is not implemented</h1>';return;}}$request=NULL;$repeatedError=FALSE;do{try{if(count($this->requests)>self::$maxLoop){throw
new
ApplicationException('Too many loops detected in application life cycle.');}if(!$request){$this->onStartup($this);$router=$this->getRouter();Nette\Debug::addPanel(new
...
instanceof
RenderResponse&&$this->isControlInvalid()){$this->response->send();$this->sendPayload();}elseif(!$this->response&&$hasPayload){$this->sendPayload();}}catch(AbortException$e){}if($this->hasFlashSession()){$this->getFlashSession()->setExpiration($this->response
instanceof
...
getFlashSession(){if(empty($this->params[self::FLASH_KEY])){$this->params[self::FLASH_KEY]=Nette\String::random(4);}return$this->getSession('Nette.Application.Flash/'.$this->params[self::FLASH_KEY]);}protected
function
getHttpRequest(){return
...
getDefaults(){return$this->defaults;}}}namespace Nette{class
DebugPanel
extends
Object
implements
...
RoutingDebugger
extends
Nette\DebugPanel{private$router;private$httpRequest;private$routers;private$request;function
__construct(IRouter$router,Nette\Web\IHttpRequest$httpRequest){$this->router=$router;$this->httpRequest=$httpRequest;$this->routers=new\ArrayObject;parent::__construct('RoutingDebugger',array($this,'renderTab'),array($this,'renderPanel'));}function
...
instanceof
Route?$router->getMask():NULL,'request'=>$request);}}}namespace Nette\Caching{use
Nette;class
Cache
extends
Nette\Object
...
ICacheJournal{const
FILE='btfj.dat';const
FILEMAGIC=0x6274666A;const
INDEXMAGIC=0x696E6465;const
DATAMAGIC=0x64617461;const
NODESIZE=4096;const
BITROT=12;const
HEADERSIZE=4096;const
INT32SIZE=4;const
USEJSON=FALSE;const
INFO='i',TYPE='t',ISLEAF='il',PREVNODE='p',END='e',MAX='m',INDEXDATA='id',LASTINDEX='l';const
TAGS='t',PRIORITY='p',ENTRIES='e';const
DATA='d',KEY='k',DELETED='d';public
static$debug=FALSE;private$file;private$handle;private$lastNode=2;private$lastModTime=NULL;public$nodeCache=array();private$nodeChanged=array();private$toCommit=array();private$deletedLinks=array();private$dataNodeFreeSpace=array();private
...
new\InvalidStateException("Cannot create journal file $this->file.");}}else{$writen=fwrite($init,pack('N2',self::FILEMAGIC,$this->lastNode));fclose($init);if($writen!==self::INT32SIZE*2){throw
...
new\InvalidStateException('Cannot acquite shared lock on journal.');}$header=stream_get_contents($this->handle,2*self::INT32SIZE,0);flock($this->handle,LOCK_UN);list(,$fileMagic,$this->lastNode)=unpack('N2',$header);if($fileMagic!==self::FILEMAGIC){fclose($this->handle);throw
...
as$prior=>$data){if($prior===self::INFO){continue;}elseif($prior>$priority){break;}$this->arrayAppendKeys($allData,$this->mergeIndexData($data));}$nodeInfo=$node[self::INFO];while($nodeInfo[self::PREVNODE]!==-1){$nodeId=$nodeInfo[self::PREVNODE];$node=$this->getNode($nodeId);if($node===FALSE){if(self::$debug)throw
...
as$link=>$foo){if(isset($masterNode[$key][$link])){unset($masterNode[$key][$link],$links[$link]);}}if(!empty($links)&&isset($masterNode[$key][self::INDEXDATA])){$this->cleanIndexData($masterNode[$key][self::INDEXDATA],$links,$masterNode[$key]);}if(empty($masterNode[$key])){unset($masterNode[$key]);}unset($toDelete[$key]);}else{break;}}$this->saveNode($masterNodeId,$masterNode);}}}private
function
mergeIndexData(array$data){while(isset($data[self::INDEXDATA])){$id=$data[self::INDEXDATA];unset($data[self::INDEXDATA]);$childNode=$this->getNode($id);if($childNode===FALSE){if(self::$debug)throw
new\InvalidStateException("Cannot load node number $id.");break;}$this->arrayAppendKeys($data,$childNode[self::INDEXDATA]);}return$data;}private
function
...
as$link=>$foo){if(isset($node[self::INDEXDATA][$link])){unset($node[self::INDEXDATA][$link],$links[$link]);}}if(isset($node[self::INDEXDATA][self::INDEXDATA])){$nextNodeId=$node[self::INDEXDATA][self::INDEXDATA];}else{$nextNodeId=FALSE;}if(empty($node[self::INDEXDATA])||(count($node[self::INDEXDATA])===1&&$nextNodeId)){if($prev===-1){if($nextNodeId===FALSE){unset($masterNodeLink[self::INDEXDATA]);}else{$masterNodeLink[self::INDEXDATA]=$nextNodeId;}}else{$prevNode=$this->getNode($prev);if($prevNode===FALSE){if(self::$debug)throw
new\InvalidStateException("Cannot load node number $prev.");}else{if($nextNodeId===FALSE){unset($prevNode[self::INDEXDATA][self::INDEXDATA]);if(empty($prevNode[self::INDEXDATA])){unset($prevNode[self::INDEXDATA]);}}else{$prevNode[self::INDEXDATA][self::INDEXDATA]=$nextNodeId;}$this->saveNode($prev,$prevNode);}}unset($node[self::INDEXDATA]);}else{$prev=$nodeId;}$this->saveNode($nodeId,$node);}}private
function
getNode($id){if(isset($this->nodeCache[$id])){return$this->nodeCache[$id];}$binary=stream_get_contents($this->handle,self::NODESIZE,self::HEADERSIZE+self::NODESIZE*$id);if(empty($binary)){return
FALSE;}list(,$magic,$lenght)=unpack('N2',$binary);if($magic!==self::INDEXMAGIC&&$magic!==self::DATAMAGIC){if(!empty($magic)){if(self::$debug)throw
new\InvalidStateException("Node $id has malformed header.");$this->deleteNode($id);}return
FALSE;}$data=substr($binary,2*self::INT32SIZE,$lenght-2*self::INT32SIZE);if(self::USEJSON){$node=@json_decode($data,TRUE);}else{$node=@unserialize($data);}if($node===FALSE){$this->deleteNode($id);if(self::$debug)throw
...
new\InvalidStateException("Cannot load node number $parentId.");}else{if($parentNode[self::INFO][self::END]===$id){if(count($parentNode)===1){$parentNode[self::INFO][self::END]=-1;}else{end($parentNode);$lastKey=key($parentNode);$parentNode[self::INFO][self::END]=$parentNode[$lastKey];unset($parentNode[$lastKey]);}}else{unset($parentNode[$nodeInfo[self::MAX]]);}$this->saveNode($parentId,$parentNode);}}if($nodeInfo[self::TYPE]===self::PRIORITY){if($nodeInfo[self::MAX]===-1){if($nodeInfo[self::PREVNODE]!==-1){$prevNode=$this->getNode($nodeInfo[self::PREVNODE]);if($prevNode===FALSE){if(self::$debug)throw
new\InvalidStateException('Cannot load node number '.$nodeInfo[self::PREVNODE].'.');}else{$prevNode[self::INFO][self::MAX]=-1;$this->saveNode($nodeInfo[self::PREVNODE],$prevNode);}}}else{list($nextId,$nextNode)=$this->findIndexNode($nodeInfo[self::TYPE],$nodeInfo[self::MAX]+1,NULL,$id);if($nextId!==-1&&$nextId!==$id){$nextNode[self::INFO][self::PREVNODE]=$nodeInfo[self::PREVNODE];$this->saveNode($nextId,$nextNode);}}}}$this->nodeCache[$id]=FALSE;}else{$this->nodeCache[$id]=$node;}$this->nodeChanged[$id]=TRUE;}private
function
...
TRUE;}if(self::USEJSON){$data=json_encode($node);}else{$data=serialize($node);}$dataSize=strlen($data)+2*self::INT32SIZE;$isData=$node[self::INFO][self::TYPE]===self::DATA;if($dataSize>self::NODESIZE){if($isData){throw
new\InvalidStateException('Saving node is bigger than maximum node size.');}else{$this->bisectNode($id,$node);return
FALSE;}}$this->toCommit[$id]=pack('N2',$isData?self::DATAMAGIC:self::INDEXMAGIC,$dataSize).$data;if($this->lastNode<$id){$this->lastNode=$id;}if($isData){$this->dataNodeFreeSpace[$id]=self::NODESIZE-$dataSize;}return
TRUE;}private
function
commitNode($id,$str){fseek($this->handle,self::HEADERSIZE+self::NODESIZE*$id);$writen=fwrite($this->handle,$str);if($writen===FALSE){throw
...
array($nodeId,array(self::INFO=>array(self::TYPE=>$type,self::ISLEAF=>TRUE,self::PREVNODE=>-1,self::END=>-1,self::MAX=>-1)),$parentId);}if($node[self::INFO][self::ISLEAF]||$nodeId===$childId||$node[self::INFO][self::PREVNODE]===$prevId){return
...
function
findFreeNode($count=1){$id=$this->lastNode;$nodesId=array();do{if(isset($this->nodeCache[$id])){++$id;continue;}$offset=self::HEADERSIZE+self::NODESIZE*$id;$binary=stream_get_contents($this->handle,self::INT32SIZE,$offset);if(empty($binary)){$nodesId[]=$id;}else{list(,$magic)=unpack('N',$binary);if($magic!==self::INDEXMAGIC&&$magic!==self::DATAMAGIC){$nodesId[]=$from;}}++$id;}while(count($nodesId)!==$count);if($count===1){return$nodesId[0];}else{return$nodesId;}}private
function
findFreeDataNode($size){foreach($this->dataNodeFreeSpace
as$id=>$freeSpace){if($freeSpace>$size){return$id;}}$id=self::$startNode[self::DATA];while(TRUE){if(isset($this->dataNodeFreeSpace[$id])||isset($this->nodeCache[$id])){++$id;continue;}$offset=self::HEADERSIZE+self::NODESIZE*$id;$binary=stream_get_contents($this->handle,2*self::INT32SIZE,$offset);if(empty($binary)){$this->dataNodeFreeSpace[$id]=self::NODESIZE;return$id;}list(,$magic,$nodeSize)=unpack('N2',$binary);if(empty($magic)){$this->dataNodeFreeSpace[$id]=self::NODESIZE;return$id;}elseif($magic===self::DATAMAGIC){$freeSpace=self::NODESIZE-$nodeSize;$this->dataNodeFreeSpace[$id]=$freeSpace;if($freeSpace>$size){return$id;}}++$id;}}private
function
bisectNode($id,array$node){$nodeInfo=$node[self::INFO];unset($node[self::INFO]);if(count($node)===1){$key=key($node);$dataId=$this->findFreeDataNode(self::NODESIZE);$this->saveNode($dataId,array(self::INDEXDATA=>$node[$key],self::INFO=>array(self::TYPE=>self::DATA,self::LASTINDEX=>($dataId<<self::BITROT))));unset($node[$key]);$node[$key][self::INDEXDATA]=$dataId;$node[self::INFO]=$nodeInfo;$this->saveNode($id,$node);return;}ksort($node);$halfCount=ceil(count($node)/2);list($first,$second)=array_chunk($node,$halfCount,TRUE);end($first);$halfKey=key($first);if($id<=2){list($firstId,$secondId)=$this->findFreeNode(2);$first[self::INFO]=array(self::TYPE=>$nodeInfo[self::TYPE],self::ISLEAF=>$nodeInfo[self::ISLEAF],self::PREVNODE=>-1,self::END=>-1,self::MAX=>$halfKey);$this->saveNode($firstId,$first);$second[self::INFO]=array(self::TYPE=>$nodeInfo[self::TYPE],self::ISLEAF=>$nodeInfo[self::ISLEAF],self::PREVNODE=>$firstId,self::END=>$nodeInfo[self::END],self::MAX=>-1);$this->saveNode($secondId,$second);$parentNode=array(self::INFO=>array(self::TYPE=>$nodeInfo[self::TYPE],self::ISLEAF=>FALSE,self::PREVNODE=>-1,self::END=>$secondId,self::MAX=>-1),$halfKey=>$firstId);$this->saveNode($id,$parentNode);}else{$firstId=$this->findFreeNode();$first[self::INFO]=array(self::TYPE=>$nodeInfo[self::TYPE],self::ISLEAF=>$nodeInfo[self::ISLEAF],self::PREVNODE=>$nodeInfo[self::PREVNODE],self::END=>-1,self::MAX=>$halfKey);$this->saveNode($firstId,$first);$second[self::INFO]=array(self::TYPE=>$nodeInfo[self::TYPE],self::ISLEAF=>$nodeInfo[self::ISLEAF],self::PREVNODE=>$firstId,self::END=>$nodeInfo[self::END],self::MAX=>$nodeInfo[self::MAX]);$this->saveNode($id,$second);list(,,$parent)=$this->findIndexNode($nodeInfo[self::TYPE],$halfKey);$parentNode=$this->getNode($parent);if($parentNode===FALSE){if(self::$debug)throw
...
function
headerCommit(){fseek($this->handle,self::INT32SIZE);@fwrite($this->handle,pack('N',$this->lastNode));}private
function
deleteNode($id){fseek($this->handle,0,SEEK_END);$end=ftell($this->handle);if($end<=(self::HEADERSIZE+self::NODESIZE*($id+1))){$packedNull=pack('N',0);do{$binary=stream_get_contents($this->handle,self::INT32SIZE,(self::HEADERSIZE+self::NODESIZE*--$id));}while(empty($binary)||$binary===$packedNull);if(!ftruncate($this->handle,self::HEADERSIZE+self::NODESIZE*($id+1))){throw
new\InvalidStateException("Cannot truncate journal file.");}}else{fseek($this->handle,self::HEADERSIZE+self::NODESIZE*$id);$writen=fwrite($this->handle,pack('N',0));if($writen!==self::INT32SIZE){throw
new\InvalidStateException("Cannot delete node number $id from journal.");}}}private
function
deleteAll(){if(!ftruncate($this->handle,self::HEADERSIZE)){throw
new\InvalidStateException("Cannot truncate journal file.");}}private
function
lock(){if($this->handle){if(!flock($this->handle,LOCK_EX)){throw
new\InvalidStateException('Cannot acquite exclusive lock on journal.');}if($this->lastModTime!==NULL){clearstatcache();if($this->lastModTime<@filemtime($this->file)){$this->nodeCache=$this->dataNodeFreeSpace=array();}}}}private
function
...
new\InvalidArgumentException("The '$prefix$key' item must be scalar or array, ".gettype($val)." given.");}}}}}namespace Nette\Database{use
Nette;use
...
public$maxLength=1000;public$totalTime=0;public$queries=array();public$name;function
logQuery(Statement$result,array$params=NULL){$this->totalTime+=$result->time;$this->queries[]=array($result->queryString,$params,$result->time,$result->rowCount());}function
...
getPanel(){$s='';foreach($this->queries
as$query){list($sql,$params,$time,$rows)=$query;$s.='<tr><td>'.sprintf('%0.3f',$time*1000).'</td><td class="database-sql">'.Connection::highlightSql(Nette\String::truncate($sql,self::$maxLength)).'</td><td>'.htmlSpecialChars(implode(', ',$params)).'</td><td>'.$rows.'</td></tr>';}return
empty($this->queries)?'':'<style> #nette-debug-database td.database-sql { background: white !important } </style>
...
as$key=>$value){if($key[0]==='['){unset($row[$key]);$key=substr($key,1,-1);$row[$key]=$value;}}return$row;}}}namespace Nette\Database\Reflection{use
Nette;class
DatabaseReflection
extends
Nette\Object{private$primary;private$foreign;private$table;function
...
sprintf($this->table,$name,$table);}}}namespace Nette\Database{use
Nette;class
Row
implements\ArrayAccess,\IteratorAggregate,\Countable{function
...
offsetUnset($nm){unset($this->$nm);}}}namespace Nette\Database\Selector{use
Nette;class
TableSelection
extends
Nette\Object
...
PDO;if(class_exists('PDO')){class
Statement
extends\PDOStatement{private$connection;public$time;protected
function
...
as$key=>$value){$type=gettype($value);$this->bindValue(is_int($key)?$key+1:$key,$value,isset($types[$type])?$types[$type]:PDO::PARAM_STR);}$time=microtime(TRUE);parent::execute();$this->time=microtime(TRUE)-$time;$this->connection->__call('onQuery',array($this,$params));return$this;}function
...
<script type="text/javascript">/*<![CDATA[*/var bs=document.getElementById("netteBluescreen");document.body.appendChild(bs);document.onkeyup=function(b){b=b||window.event;b.keyCode==27&&bs.onclick({target:document.getElementById("netteBluescreenIcon")})};
...
'<\/script><body id="nette-debug">');c.body.innerHTML='<div class="nette-panel nette-mode-window" id="'+d+'">'+this.dom().innerHTML+"</div>";b.Nette.Debug.Panel.factory(d).initToggler().reposition();c.title=a.find("h1").dom().innerHTML;h([b]).bind("unload",function(){a.toPeek();b.close()});h(c).bind("keyup",function(f){f.keyCode===27&&b.close()});document.cookie=d+"=window; path=/";this.hide().removeClass(e.FLOAT).removeClass(e.PEEK).addClass(e.WINDOW).data().win=b}},init:function(){var a=this,b;
a.data().onmove=function(c){var d=document,f=window.innerHeight||d.documentElement.clientHeight||d.body.clientHeight;c.left=Math.max(Math.min(c.left,0.8*this.offsetWidth),0.2*this.offsetWidth-(window.innerWidth||d.documentElement.clientWidth||d.body.clientWidth));c.top=Math.max(Math.min(c.top,0.8*this.offsetHeight),this.offsetHeight-f)};h(window).bind("resize",function(){a.reposition()});a.draggable({handle:a.find("h1"),stop:function(){a.toFloat()}}).bind("mouseenter",function(){a.focus()}).bind("mouseleave",
function(){a.blur()});this.initToggler();a.find(".nette-icons").find("a").bind("click",function(c){this.rel==="close"?a.toPeek():a.toWindow();c.preventDefault()});if(b=document.cookie.match(RegExp(a.dom().id+"=(window|(-?[0-9]+):(-?[0-9]+))")))b[2]?a.toFloat().position({left:b[2],top:b[3]}):a.toWindow();else a.addClass(e.PEEK)},initToggler:function(){var a=this;this.bind("click",function(b){var c=h(b.target).closest("a").dom();if(c&&c.rel){e._toggle(c);b.preventDefault();a.reposition()}});return this}});
Nette.Debug.Bar=Nette.Class({Extends:Nette.Q,constructor:function(){Nette.Q.call(this,"#nette-debug-bar")},init:function(){var a=this,b;a.data().onmove=function(c){var d=document,f=window.innerHeight||d.documentElement.clientHeight||d.body.clientHeight;c.left=Math.max(Math.min(c.left,0),this.offsetWidth-(window.innerWidth||d.documentElement.clientWidth||d.body.clientWidth));c.top=Math.max(Math.min(c.top,0),this.offsetHeight-f)};h(window).bind("resize",function(){a.position(a.position())});a.draggable({draggedClass:"nette-dragged",
stop:function(){document.cookie=a.dom().id+"="+a.position().left+":"+a.position().top+"; path=/"}});a.find("a").bind("click",function(c){if(this.rel==="close"){h("#nette-debug").hide();window.opera&&h("body").show()}else if(this.rel){var d=e.factory(this.rel);if(c.shiftKey)d.toFloat().toWindow();else if(d.hasClass(e.FLOAT)){var f=h(this).offset();d.offset({left:f.left-d.position().width+f.width+4,top:f.top-d.position().height-4}).toPeek()}else d.toFloat().position({left:d.position().left-Math.round(Math.random()*
100)-20,top:d.position().top-Math.round(Math.random()*100)-20}).reposition()}c.preventDefault()}).bind("mouseenter",function(){if(!(!this.rel||this.rel==="close"||a.hasClass("nette-dragged"))){var c=e.factory(this.rel);c.focus();if(c.hasClass(e.PEEK)){var d=h(this).offset();c.offset({left:d.left-c.position().width+d.width+4,top:d.top-c.position().height-4})}}}).bind("mouseleave",function(){!this.rel||this.rel==="close"||a.hasClass("nette-dragged")||e.factory(this.rel).blur()});if(b=document.cookie.match(RegExp(a.dom().id+
"=(-?[0-9]+):(-?[0-9]+)")))a.position({left:b[1],top:b[2]});a.find("a").each(function(){!this.rel||this.rel==="close"||e.factory(this.rel).init()})}})})();/*]]>*/</script>
...
function
clickableDump($dump){return'<pre class="nette-dump">'.preg_replace_callback('#^( *)((?>[^(]{1,200}))\((\d+)\) <code>#m',function($m){return"$m[1]<a href='#' rel='next'>$m[2]($m[3]) ".(trim($m[1])||$m[3]<7?'<abbr>&#x25bc;</abbr> </a><code>':'<abbr>&#x25ba;</abbr> </a><code class="nette-collapsed">');},self::htmlDump($dump)).'</pre>';}}use
...
Environment::getContext();$context->addService('Nette\\Application\\IRouter','Nette\Application\MultiRouter');if(!$context->hasService('Nette\\Application\\IPresenterLoader')){$context->addService('Nette\\Application\\IPresenterLoader',function(){return
new
Nette\Application\PresenterLoader(Environment::getVariable('appDir'));});}$class=isset($options['class'])?$options['class']:'Nette\Application\Application';$application=new$class;$application->setContext($context);$application->catchExceptions=Environment::isProduction();return$application;}static
function
...
instanceof
Nette\Web\Html){$dest->add((string)$value2->selected(isset($selected[$key2])));}else{$key2=$this->useKeys?$key2:$value2;$value2=$this->translate($value2);$dest->add((string)$option->value($key2===$value2?NULL:$key2)->selected(isset($selected[$key2]))->setText($value2));}}}return$control;}}class
MultiSelectBox
extends
SelectBox{function
...
register(){$cache=$this->getCache();$key=$this->getKey();if(isset($cache[$key])){$this->list=$cache[$key];}else{$this->rebuild();}parent::register();}function
...
addEmbeddedFile($file,$content=NULL,$contentType=NULL){return$this->inlines[$file]=$this->createAttachment($file,$content,$contentType,'inline')->setHeader('Content-ID','<'.String::random().'>');}function
...
instanceof
Nette\Application\Control){foreach($control->getComponents(FALSE,'Nette\Application\Control')as$child){if($child->isControlInvalid()){$child->render();}}}}}final
class
...
isset($this->registry[$lower])||(!$created&&isset($this->factories[$lower]));}}use
...
REVISION='9f535f9 released on 2011-01-10';public
static$iAmUsingBadHost=FALSE;final
function
__construct(){throw
new\LogicException("Cannot instantiate static class ".get_class($this));}}class
Image
extends
Object{const
ENLARGE=1;const
...
isset($types[$type])?$types[$type]:NULL;return
NULL;}static
function
...
function
error($message="Unexpected '%s'"){list(,$line,$col)=self::$tokenizer->getOffset($this->n);$token=str_replace("\n",'\n',Nette\String::truncate(self::$tokenizer->tokens[$this->n],40));throw
new
NeonException(str_replace('%s',$token,$message)."' on line ".($line-1).", column $col.");}}final
class
ObjectMixin{private
...
new\MemberAccessException("Cannot write to a read-only property $class::\$$name.");}}$name=func_get_arg(1);throw
new\MemberAccessException("Cannot write to an undeclared property $class::\$$name.");}static
function
...
new\InvalidStateException('Critical section has already been entered.');}$handle=@fopen(NETTE_DIR.'/lockfile','r')?:fopen(NETTE_DIR.'/lockfile','w');if(!$handle){throw
new\InvalidStateException("Unable initialize critical section (missing file '".NETTE_DIR."/lockfile').");}flock(self::$criticalSections=$handle,LOCK_EX);}static
function
leaveCriticalSection(){if(!self::$criticalSections){throw
new\InvalidStateException('Critical section has not been initialized.');}flock(self::$criticalSections,LOCK_UN);fclose(self::$criticalSections);self::$criticalSections=NULL;}}}namespace Nette\Web{use
Nette;class
Html
extends
Nette\Object
